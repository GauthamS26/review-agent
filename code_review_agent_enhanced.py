#!/usr/bin/env python3
"""Enhanced Code Review Agent with better output formatting."""
...
def colored(text: str, color: str = Fore.WHITE, style: str = Style.NORMAL) -> str:
...
def print_header(text: str) -> None:
    ...
    print(colored(text, Fore.CYAN, Style.BRIGHT))
...
def print_section(text: str) -> None:
    ...
    print(colored("-" * 80, Fore.GREEN))


def print_info(text: str) -> None:
    """Print info message."""
    print(colored(f"[INFO] {text}", Fore.BLUE))


def print_success(text: str) -> None:
    """Print success message."""
    print(colored(f"[✓] {text}", Fore.GREEN))


def print_error(text: str) -> None:
    """Print error message."""
    print(colored(f"[✗] {text}", Fore.RED, Style.BRIGHT), file=sys.stderr)


def print_warning(text: str) -> None:
    """Print warning message."""
    print(colored(f"[!] {text}", Fore.YELLOW, Style.BRIGHT))
...
def clone_git_repo(git_url: str, temp_dir: str) -> Path:
    ...
    print_info(f"Cloning repository from {git_url}...")
    try:
...
def process_code_review(source_path: str, is_git: bool = False, 
    ...
    try:
        ...
        if is_git:
            ...
            print_info(f"Using directory: {code_dir}")
        
        # Collect code files
        print_section("COLLECTING CODE FILES")
        code_files = collect_code_files(code_dir)
        if not code_files:
            raise ValueError("No code files found in the provided directory.")
        
        print_success(f"Found {len(code_files)} code files")
        
        # Show files being reviewed
        print_info("Files to be reviewed:")
        ...
        print_section("PREPARING CODE ANALYSIS")
        code_context = prepare_code_context(code_files)
        
        # Get code review from Ollama
        print_section("ANALYZING CODE WITH LLAMA MODEL")
        print_warning("This may take several minutes depending on model size and system resources...")
...
"""
        ...
        if output_file:
            with open(output_file, "w", encoding="utf-8") as f:
                f.write(output)
            print_success(f"Review saved to: {output_file}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=colored("Code Review Agent using Ollama and Llama Model", Fore.CYAN, Style.BRIGHT),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
EXAMPLES:
  Review a local folder:
    python code_review_agent_enhanced.py /path/to/my/project
  
  Review a git repository:
    python code_review_agent_enhanced.py https://github.com/user/repo --git
  
  Review with custom model and save output:
    python code_review_agent_enhanced.py /path/to/project --model llama2:13b --output review.txt
  
  Review with custom Ollama host:
    python code_review_agent_enhanced.py /path/to/project --ollama_host http://localhost:11434
        """
    )
